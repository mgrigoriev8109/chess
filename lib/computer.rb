...
module Computer
  def determine_computer_movement(color, board)
    ...
    if verify_check(color, board)
      find_king_escape(color, board)
    elsif find_computer_checkmate(color, board).any?
      find_computer_checkmate(color, board)
    elsif find_computer_check(color, board).any?
      find_computer_check(color, board)
    elsif find_computer_attack(color, board).any?
      find_computer_attack(color, board)
    else
      find_computer_move(color, board)
    end
  end
  ...
  def find_computer_checkmate(color, board)
    ...
    board.each_with_index do |row, row_index|
      row.each_with_index do |cell, column_index|
        ...
        next unless cell.is_a?(Piece) && cell.color == color && move_results_in_checkmate(color, current_coordinates,
                                                                                          board)
  ...
  def move_results_in_checkmate(color, starting_coordinates, board)
    ...
    all_movements.each do |possible_end|
      ...
      move_resulting_in_checkmate = possible_end if assess_endofround_checkmate(color, simulated_board)
    end

    move_resulting_in_checkmate = false if starting_piece.is_a?(King)
  ...
  def find_computer_check(color, board)
    ...
    board.each_with_index do |row, row_index|
      row.each_with_index do |cell, column_index|
        current_coordinates = [row_index, column_index]
        next unless cell.is_a?(Piece) && cell.color == color && move_results_in_check(color, current_coordinates, board)
  ...
  def move_results_in_check(color, starting_coordinates, board)
    ...
    all_movements.each do |possible_end|
      simulated_board = Marshal.load(Marshal.dump(board))
      move_gamepiece(starting_coordinates, possible_end, simulated_board)
      move_resulting_in_check = possible_end if verify_check(opposite_player_color(color), simulated_board)
    end

    move_resulting_in_check = false if starting_piece.is_a?(King)
  ...
  def find_computer_attack(color, board)
    ...
    board.each_with_index do |row, row_index|
      row.each_with_index do |cell, column_index|
        current_coordinates = [row_index, column_index]
        next unless cell.is_a?(Piece) && cell.color == color && cell.all_possible_attacks(board,
  ...
  def piece_locations_of_a_player(color, board)
    ...
    board.each_with_index do |row, row_index|
      row.each_with_index do |cell, column_index|
        current_coordinates = [row_index, column_index]
        all_piece_locations.push(current_coordinates) if cell.is_a?(Piece) && cell.color == color
      end
    end
    all_piece_locations
  end
